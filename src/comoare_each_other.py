    ...
    with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            for idx, img in enumerate(image_paths):
                ...
                print img
                print("开始")
                for idx2, img2 in enumerate(image_paths):
                    if idx >= idx2:
                        continue
                    embedding2 = compute_facial_encoding(sess,images_placeholder,embeddings,phase_train_placeholder,image_size, embedding_size,np.zeros((1, embedding_size)),img2) 
                    print img2
                    print('距离：')
                    dis = face_distance(embedding, embedding2)
                    print dis
                    if dis < 0.75:
                        print("同一个人")
                    else:
